...
/* built_ins */
unsigned int findBuiltIns(config *build);
...
/* cd */
int cdFunc(config *);
unsigned int cdToHome(config *build);
unsigned int cdToPrevious(config *build);
unsigned int cdToCustom(config *build);
unsigned int updateEnviron(config *build);

/* cd2 */
int updateOld(config *build);
unsigned int updateCur(config *build, int index);
...
/* split_string */
unsigned int splitString(config *build);
unsigned int countWords(char *s);
unsigned int isSpace(char c);
